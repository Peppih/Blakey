    ...
    <div style={{ minHeight:"100vh", background: CREAM }}>
      ...
      <header style={{
        ...
        <div style={{
          ...
          background:"radial-gradient(circle,rgba(201,150,62,.14) 0%,transparent 70%)", pointerEvents:"none"
        }}/>
        ...
        <div style={{ maxWidth:840, margin:"0 auto", display:"flex", gap:28, alignItems:"center", flexWrap:"wrap" }}>
          ...
          <div style={{ flex:1, minWidth:180 }}>
            ...
            <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:clamp(28,4.5,38), fontWeight:700, color:WHITE, lineHeight:1.15, marginBottom:8 }}>
              The Blakey<br/>
      ...
      <main style={{ maxWidth:840, margin:"0 auto", padding:"24px 16px 64px" }}>
              ...
              {isOpen && (
                <div style={{ borderTop:"1px solid #F0EAE0" }}>
                  {visible.map((t, i) => {
                    ...
                    const W = WHO[t.who];
                    return (
                      <div key={t.id} className={`task-row${done?" done-row":""}`}
                        onClick={() => toggle(t.id)}
                        style={{
                        ...
                        <div className="chk" style={{
                          ...
                          border: done ? "none" : "2px solid #CEBFA8",
                          background: done ? "#1E6B47" : "transparent",
                        ...
                        {/* Content */}
                        <div style={{ flex:1, minWidth:0 }}>
                          <p className={done ? "done-text" : ""} style={{ fontSize:13, color:"#222", lineHeight:1.55, marginBottom:6 }}>
                            {t.text}
                          </p>
                            ...
                            {t.link && (
                              <a href={t.link.url} target="_blank" rel="noopener noreferrer"
                                ...
                                <svg width="9" height="9" viewBox="0 0 9 9" fill="none">
                                  <path d="M1 8L8 1M8 1H3M8 1V6" stroke="#1455A4" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                                </svg>
                                {t.link.label}
                              </a>
                            )}
        ...
        {/* Legend */}
        <div style={{ marginTop:28, padding:"18px 20px", borderRadius:14, background:WHITE, border:"1px solid #E3DDD3" }}>
          <p style={{ fontSize:10, fontWeight:700, color:"#AAA", textTransform:"uppercase", letterSpacing:2, marginBottom:10 }}>Legend</p>
          <div style={{ display:"flex", gap:14, flexWrap:"wrap", alignItems:"center" }}>
            {Object.entries(WHO).map(([k,v]) => (
              <div key={k} style={{ display:"flex", alignItems:"center", gap:6 }}>
                <span style={{ fontSize:10, fontWeight:700, padding:"2px 9px", borderRadius:10, background:v.bg, color:v.fg, textTransform:"uppercase", letterSpacing:.8 }}>{v.label}</span>
                <span style={{ fontSize:11, color:"#AAA" }}>tasks</span>
              </div>
            ))}
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              <span style={{ fontSize:10, fontWeight:700, padding:"2px 10px", borderRadius:10, background:"#E8F0FD", color:"#1455A4" }}>↗ Link</span>
              <span style={{ fontSize:11, color:"#AAA" }}>opens official form or portal</span>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              <div style={{ width:16, height:16, borderRadius:4, background:"#1E6B47", display:"flex", alignItems:"center", justifyContent:"center" }}>
                <svg width="9" height="8" viewBox="0 0 9 8" fill="none"><path d="M1 4L3.5 6.5L8 1" stroke="white" strokeWidth="1.8" strokeLinecap="round"/></svg>
              </div>
              <span style={{ fontSize:11, color:"#AAA" }}>task completed</span>
            </div>
          </div>
        </div>
      </main>
